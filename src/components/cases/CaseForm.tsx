...
const CaseForm: React.FC<CaseFormProps> = ({ case: existingCase, onSubmit, onImport, onCancel, iscopying }) => {
  ...
  const handleLLMAnalysis = async () => {
    ...
    try {
      ...
      console.log('Starting LLM analysis with content:', emailContent.substring(0, 100) + '...');
      const llmResult = await llmService.analyzeEmail(emailContent);
      
      if (llmResult.success && llmResult.extractedData) {
        const extractedData = llmResult.extractedData;
        const resultMessage = `✅ LLM解析完了！信頼度: ${Math.round((llmResult.confidence || 0) * 100)}%`;
        ...
        const errorMessage = llmResult.error || 'Unknown error';
        setParseResult(`❌ LLM解析に失敗しました: ${errorMessage}`);
        console.error('LLM analysis failed:', llmResult.error);
      }
    } catch (err) {
